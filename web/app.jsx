...
// ==========================================
function Icon({ name, className = "icon-svg", size = 16 }) {
  const s = size;
  const base = { width: s, height: s, display: "block", flexShrink: 0 };
  const icons = {
    compass: (
      <svg style={base} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    star: (
      <svg style={base} viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  const el = icons[name];
  if (!el) return null;
  // Clone element applying className via a wrapper span so CSS classes still work
  return <span className={className} style={{ display: "inline-flex", alignItems: "center", flexShrink: 0, lineHeight: 0 }}>{el}</span>;
}
...
function RideSearchForm({ search, setSearch, onSubmit, compact }) {
  return (
    <form className={`ride-search-grid-layout ${compact ? "compact" : ""}`} onSubmit={onSubmit}>
      <label className="input-group">
        <span>Pickup</span>
        <input type="text" value={search.from} onChange={(e) => setSearch({ ...search, from: e.target.value })} placeholder="e.g. Jayanagar 4th Block" required />
      </label>

      <label className="input-group">
        <span>Destination</span>
        <input type="text" value={search.to} onChange={(e) => setSearch({ ...search, to: e.target.value })} placeholder="e.g. RVCE Mysore Road" required />
      </label>
